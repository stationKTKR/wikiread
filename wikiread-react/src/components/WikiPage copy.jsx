...
function WikiPage({ setEnding }) {
    ...
    const [questsdone, setquestsdone] = useState(4);
    ...
    const bigpages = ["Superorganism", "Wikipedia", "Global brain", "Information hazard", "Observer effect (physics)", "Abiogenesis"];

    function allowclearalert() {
        setallowclear(true)
    }

    function puzWave() {
        clearTimeout(puztalktimeout);
        setleftpuzhandStyle({
            position: "relative",
            top: "-90px",
            left: "45px",
        ...
        puztalktimeout = setTimeout(puzx2, 3000, "wave");
    }
    ...
    function puzwander() {
        ...
        if (puznewtalkRef.current < 2) {
            ...
            if (willwander == 4) {
                ...
                puztalktimeout = setTimeout(puzx2, 3000, "wander");
            }
        }
    }
    ...
    function puztalk(say) {
        ...
        puztalktimeout = setTimeout(puzx2, 3000, "talking " + say);
    }
    ...
    function puzmove() {
        ...
        if (puzmoveRef.current % 2 === 0) {
            ...
            puztalktimeout = setTimeout(puzx2, 3000, "move1");
            ...
            if (puzmoveRef.current > 12) {
                ...
                puztalktimeout = setTimeout(puzx2, 3000, "move2");
            } else {
                let newx = Math.floor(Math.random() * 100);
                let newy = Math.floor(Math.random() * -60);
                setpuzboxanimate({ x: newx + "vh", y: newy + "vh" });
        ...
        if (allPagesRef.current.length === 0) {
            ...
            if (allPagesRef.current.find(({ title }) => title === newPage).visited === 0) {
                ...
                if (bigpages.indexOf(newPage) > -1) {
                    console.log("bigpage");
                    setwikiAlert(true);
                    setTimeout(allowclearalert, 1000);
                }


                if (questRef.current[4].indexOf(newPage) > -1 && questRef.current[4].length == 5) {
                    //console.log("here");
                    setAbnormQuest(true);
        ...
        <Container className="contain2 container-fluid container">
            ...
            <Row className="row2">
                ...
                <Col>
                    ...
                    {wikiAlert &&
                        <div onMouseOver={clearAlert}><WikiAlert pagename={currentpage}></WikiAlert></div>
                    }
